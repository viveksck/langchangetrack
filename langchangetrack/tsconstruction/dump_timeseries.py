...
def CreateWordTimeSeries(old_df, new_df, w, sourcexinter, destxinter, useEuclidean = False, interpolate=False):
    from scipy.interpolate import interp1d
    from scipy.interpolate import UnivariateSpline
    sourcex = np.asarray(old_df[old_df.w == w].s.values, dtype=int)
    destx = np.asarray(new_df[new_df.w == w].s.values, dtype=int)
    print "SOURCEX", sourcex, w
    print "DESTX", destx, w
    if useEuclidean == False:
        old_values = old_df[old_df.w == w].cosine.values
        new_values = new_df[new_df.w == w].cosine.values
    else:
        old_values = old_df[old_df.w == w].euclidean.values
        new_values = new_df[new_df.w == w].euclidean.values
    try:
...
def process_chunk(chunk, func, olddf, newdf, sourcexinter, destxinter, useEuclidean, interpolate):
  results = [func(olddf, newdf, e, sourcexinter, destxinter, useEuclidean, interpolate) for e in chunk]
  return results
...
def main(args):
  ...
  words = pd.Series(olddf.w.values.ravel()).unique()
  oldrows = []
  newrows = []
  ...
  print sourcexinter, destxinter
  assert(len(sourcexinter) == len(destxinter))
  words_chunks = more_itertools.chunked(words, 500)
  timeseries_chunks = Parallel(n_jobs=16,
verbose=20)(delayed(process_chunk)(chunk, CreateWordTimeSeries, olddf, newdf,
sourcexinter, destxinter, useEuclidean=False, interpolate=args.interpolate) for chunk in words_chunks)
